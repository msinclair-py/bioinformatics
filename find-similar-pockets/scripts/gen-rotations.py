...
def make_pdb (array, deg, tilt):
	with open('../initial_structures/nitrogenase'+'_'+str(deg)+str(tilt), 'w') as f:
		with open('../initial_structures/cent_pocket.pdb.aligned', 'r') as infile:
			t=-1
			for line in infile:
				if "CRYST1" in line:
					f.write(line)
...
def orient (array, residues):
	fpath = '../initial_structures/cent_pocket.pdb'
	res1 = residues[0]+1
	res2 = residues[1]+1
...
#pass the normalized vector to vmd align.tcl script to do aligning	
	...
	args = ['vmd','-dispdev','text','-e','align.tcl','-args',na,a1,a2,a3]
...
coords = np.genfromtxt('../initial_structures/cent_pocket.pdb', dtype=float, skip_header=1,
...
aligned_coords = np.genfromtxt('../initial_structures/cent_pocket.pdb.aligned', dtype=float,
